...
if __name__ == "__main__":
    ...
    path = '/home/sruiz/datasets/deeplabcut/kalo_v2_imgs_20-11-2020'
    ...
    if os.path.isdir(path):
        ...
        for img_p in tqdm(imgs):
            ...
            if curr_frame is not None and prev_frame is not None:
                ...
                frame = Frame(i, img, count)
                frames.append(frame)
            prev_frame = curr_frame
            i += 1

    elif os.path.isfile(path):
        # assume file is a video
    ...
    if USE_TOP_ORDER:
        ...
        for keyframe in frames[:NUM_TOP_FRAMES]:
            ...
            cv2.imwrite(os.path.join(output_dir, name), keyframe.frame)
    ...
    if USE_THRESH:
        ...
        for i in range(1, len(frames)):
            if (rel_change(np.float(frames[i - 1].value), np.float(frames[i].value)) >= THRESH):
                ...
                cv2.imwrite(os.path.join(output_dir, name), frames[i].frame)
    ...
    if USE_LOCAL_MAXIMA:
        ...
        print("Number of keyframes:", len(frame_indexes))
        print("Frame indexes:", frame_indexes)
        for i in frame_indexes:
            name = "frame_" + str(frames[i - 1].id) + ".jpg"
            # cv2.imwrite(os.path.join(output_dir, name), frames[i - 1].frame)
